...
@section('titulo', 'Editar Responsável Técnico')
...
<div style="max-width:720px">
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:20px;font-size:.82rem;color:#64748b">
        ...
        <span>/</span>
        <span style="color:#1e293b">Editar Responsável</span>
    </div>

    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:28px">
        <h2 style="font-size:1rem;font-weight:700;color:#1e293b;margin-bottom:6px">Editar Responsável Técnico</h2>
        <p style="font-size:.82rem;color:#64748b;margin-bottom:24px">{{ $responsavel->nome }}</p>

        <form method="POST" action="{{ route('responsaveis.update', [$empresaElaboradora, $responsavel]) }}">
            @csrf
            @method('PUT')
