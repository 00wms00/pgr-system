...
<div style="max-width:760px">
    <div style="margin-bottom:20px">
        ...
        <h2 style="font-size:1.1rem;font-weight:700;color:#1e293b;margin:8px 0 2px">Nova Avaliação de Risco</h2>
        <p style="font-size:.8rem;color:#64748b;margin:0">
            {{ $risco->agente ?? $risco->fonte_geradora }}
            &mdash; GHE {{ $risco->ghe->nome }} / {{ $risco->ghe->setor->nome }}
        </p>
    </div>
    <div style="background:#fff;border-radius:10px;border:1px solid #e2e8f0;padding:24px">
        <form method="POST" action="{{ route('riscos.avaliacoes.store', $risco) }}">
            @csrf
