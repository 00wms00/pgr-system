...
<div style="max-width:640px">
    <div style="margin-bottom:20px">
        <a href="{{ route('ghes.show', $ghe) }}"
            ...
            Voltar para {{ $ghe->nome }}
        </a>
        <h2 style="font-size:1.1rem;font-weight:700;color:#1e293b;margin:8px 0 2px">Editar GHE</h2>
        <p style="font-size:.8rem;color:#64748b;margin:0">{{ $ghe->nome }} &mdash; {{ $ghe->setor->nome }}</p>
    </div>
    <div style="background:#fff;border-radius:10px;border:1px solid #e2e8f0;padding:24px">
        <form method="POST" action="{{ route('ghes.update', $ghe) }}">
            @csrf @method('PUT')
            @include('ghes._form')
