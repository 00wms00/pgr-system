...
class GheController extends Controller
{
    ...
    {
        $ghes = Ghe::whereHas('setor.unidade', function ($q) {
            $q->where('empresa_id', auth()->user()->empresa_id);
        })->with('setor.unidade')->orderBy('nome')->paginate(20);
    ...
    public function store(GheRequest $request)
    {
        ...
        return redirect()->route('ghes.index')->with('success', 'GHE criado com sucesso.');
    }

    public function show(Ghe $ghe)
    {
        $ghe->load('setor.unidade', 'riscosInventario.riscoTipo');
    ...
    public function update(GheRequest $request, Ghe $ghe)
    {
        ...
        return redirect()->route('ghes.index')->with('success', 'GHE atualizado.');
    }

    public function destroy(Ghe $ghe)
    {
        $ghe->delete();
        return redirect()->route('ghes.index')->with('success', 'GHE removido.');
    }
}
